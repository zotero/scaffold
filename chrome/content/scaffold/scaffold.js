...
var Scaffold = new function() {
	...
	function load() {
		...
		var m = /^\s*{[\S\s]*?}\s*?[\r\n]+/.exec(header);
		var fixedCode = translator.code.substr(m[0].length).replace(/^\/\* FW LINE [^\n]*\n/,'\n');
	...
	function _getTranslator() {
		...
		translator.code = "1;" + _FW + document.getElementById('editor-code').textbox.value;
